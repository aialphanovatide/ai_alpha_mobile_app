  ...
  return (
    <List.Section title="Analyses" titleStyle={styles.mainTitle}>
      <List.Accordion
        ...
        onPress={handlePress}>
        {stories.map((story) => (
          <AnalysisItem key={story.id} title={story.title} description={story.description} image={story.image} />
        ))}
      </List.Accordion>
    </List.Section>
  );
